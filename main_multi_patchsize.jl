...
function julia_main(magnitude_nii_file::String,phase_nii_file::String,partiallyUnwrapped_nii_file::String,criticalROI_nii_file::String,ChooseThisSlice::Int64,criticalROILabel::Int64)
    ...
    patchsizelist = Array(7:2:15);

    pxmapALL = SharedArray{Float64,3}((datasize1,datasize2,length(patchsizelist)));
    pymapALL = SharedArray{Float64,3}((datasize1,datasize2,length(patchsizelist)));
    # pxmapALL = zeros(datasize1,datasize2,length(patchsizelist));
    # pymapALL = zeros(datasize1,datasize2,length(patchsizelist));

    @time @sync @distributed for countme = 1:length(patchsizelist)
    ...
    pxmap = pxmap_checked[:,:,1]; # use the data from patch size = 7 (see our journal paper)
    pymap = pymap_checked[:,:,1]; # use the data from patch size = 7 (see our journal paper)
    ...
    figure(1,figsize=(12,6));imshow(reverse(permutedims(vcat(preludephasemap,newPhaseMapMask),[2,1]),dims=1),cmap="hot",interpolation="none"); axis("off"); 
